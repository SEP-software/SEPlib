...
float uxscale = 0.0, uyscale = 0.0;   /* transmitted w/vplot */
char *txbuffer;
...
int fatbase = 0;
float epause = 0.0f; /* time to pause before erasing screen */
...
char interact[MAXFLEN + 1] = ""; /* Where to store coordinate
						 * file */
int interactcoords = 1;          /* 1=user, 2=vplot */
 ...
 */
    ...
    if (stringptr != NULL)
    {
        ...
        else if ((stringptr[0] == 'o') || (stringptr[0] == 'O'))
            default_style = OLD;
        else if ((stringptr[0] == 'a') || (stringptr[0] == 'A'))
 ...
 */
    ...
    mxx = (float)cos(2. * 3.1415926535 * rotate / 360.);
    myy = (float)cos(2. * 3.1415926535 * rotate / 360.);
    mxy = (float)sin(2. * 3.1415926535 * rotate / 360.);
    myx = (float)-sin(2. * 3.1415926535 * rotate / 360.);
