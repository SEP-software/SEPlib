...
#define MY_SEP_DOC                                                            \
  ...
  sep_add_doc_line(                                                           \
      "    Csv2Sep - import SEP gathers from comma-separated value (CSV) "    \
      "files");                                                               \
  sep_add_doc_line("");                                                       \
  ...
  sep_add_doc_line(                                                           \
      "    spreadsheet data. This utility reads each 2D gather in the");      \
  sep_add_doc_line("    input CSV dataset into an SEP dataset.");             \
  sep_add_doc_line(                                                           \
      "    If the input has trace headers, they, too, are copied to the");    \
...
#define CRLF "\015\012"
  ...
  if (1 > auxpar("nrec", "d", &nrec, "csvpar")) (void)getch("nrec", "d", &nrec);
    ...
    if (csvfp != ((FILE *)NULL)) {
      if (0 == strncmp(orient, "inc", 3)) { /* traces go down columns */
        ...
        while (((char *)NULL) != fgets(tempbuf, maxlinelen + 1, csvfp)) {
          tokarg = &(tempbuf[0]);
          tok = strtok(tokarg, "\t ,");
          if (tok == ((char *)NULL)) /* something is really wrong? */
            break;
          if (0 == sscanf(tok, "%lg", &dtemp)) {
            ...
            if (ikey >= nkeys) { /* new header name */
              ...
              keytype[nkeys] = (NULL == strchr(tok + strlen(tok) + 1, '.'))
                                   ? INTKEY
                                   : FLOATKEY;
              nkeys++;
  ...
  if (nkeys > 0) {
    ...
    if (rc > 0) {
      ...
      if (((FILE *)NULL) == auxout(stdoutname)) {
        sepwarn(3,
                "Unable to create output trace header file \"%s\"\n"
      ...
      if (tempbuf[0] == '\0') {
        sepwarn(4,
                "Trouble trying to create output trace header file.\n"
                "Will continue without creating trace headers.\n");
        nkeys = 0;
      } else {
  ...
  if (!sep77) {
    ...
    for (j = 0; j < n2; ++j) {
      ...
      if (outhdrs[j] == ((int *)NULL)) seperr("Unable to allocate memory!\n");
    }
  } else {
    nbytesHdr = 0;
  }
  ...
  if (0 == strncmp(orient, "inc", 3)) { /* in columns */
    ...
    for (irec = 0; irec < nrec; ++irec) {
      ...
      if (!sep77)
        for (j = 0; j < n2; ++j)
          memset(outhdrs[j], '\0', nkeys * sizeof(float));
      ...
      if (!sep77) { /* write out trace headers */
        ...
        for (j = 0; j < n2; ++j)
          if (rc != srite(stdoutname, outhdrs[j], rc)) {
      ...
      for (j = 0; j < n2; ++j)
        if (rc != srite("out", outtraces[j], rc)) {
