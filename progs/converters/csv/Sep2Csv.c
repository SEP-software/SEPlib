...
#define MY_SEP_DOC                                                             \
  ...
  sep_add_doc_line(                                                            \
      "    Sep2Csv - export SEP gathers to comma-separated value (CSV) "       \
      "files");                                                                \
  sep_add_doc_line("");                                                        \
  ...
  sep_add_doc_line(                                                            \
      "                  parfile created to read back csv file(s)");           \
  ...
  sep_add_doc_line(                                                            \
      "    The Comma-Separated Value text file format (RFC 4180) is widely "   \
      "accepted");                                                             \
  sep_add_doc_line(                                                            \
      "    by many software packages as a way to deal with matrix-like");      \
  sep_add_doc_line(                                                            \
      "    spreadsheet data. This utility copies each 2D gather in the");      \
  sep_add_doc_line(                                                            \
      "    input SEP dataset to a separate CSV file in a designated "          \
      "directory.");                                                           \
  sep_add_doc_line(                                                            \
      "    If the input data have trace headers, they, too, are copied to "    \
      "the");                                                                  \
  sep_add_doc_line(                                                            \
      "    CSV file. The CSV files are numbered by their (multi-)index");      \
  sep_add_doc_line(                                                            \
      "    in the input, i.e. {outpref}_{iN}_..._{i3}.csv for N-dimensional "  \
      "input. ");                                                              \
  sep_add_doc_line("");                                                        \
  sep_add_doc_line(                                                            \
      "    The format of an output CSV file for an input with trace headers "  \
      "is:");                                                                  \
  sep_add_doc_line("");                                                        \
  ...
  sep_add_doc_line("    ...                                    ");             \
  sep_add_doc_line(                                                            \
      "    <blank>,trace1[1],trace2[1],trace3[1],...,traceN2[1]");             \
  sep_add_doc_line(                                                            \
      "    <blank>,trace1[2],trace2[2],trace3[2],...,traceN2[2]");             \
  sep_add_doc_line("    ...                                    ");             \
  sep_add_doc_line(                                                            \
      "    <blank>,trace1[N1],trace2[N1],trace3[N1],...,traceN2[N1]");         \
  sep_add_doc_line("");                                                        \
  sep_add_doc_line(                                                            \
      "    The format of an output CSV file for an input without trace "       \
      "headers is:");                                                          \
  sep_add_doc_line("");                                                        \
...
static void gotowrapup(int signum) { wrapup = 1; }
...
#define CRLF "\015\012"
  ...
  outpref[0] = '.';
  (void)datapath(outpref);
  strcat(outpref, "/gather");
  ...
  dimlist[0] = n1;
  dimlist[1] = n2;
  if (0 >= fetch("n3", "d", dimlist + 2))
    ndims = 2;
  else if (0 >= fetch("n4", "d", dimlist + 3))
    ndims = 3;
  else if (0 >= fetch("n5", "d", dimlist + 4))
    ndims = 4;
  else if (0 >= fetch("n6", "d", dimlist + 5))
    ndims = 5;
  else if (0 >= fetch("n7", "d", dimlist + 6))
    ndims = 6;
  else
    ndims = 7;
  ...
  fprintf(csvparfp, "outpref=\"%s\"%sorient=\"%s\"%sndims=%d%s", outpref, CRLF,
          orient, CRLF, ndims, CRLF);
  ...
  if (verbose)
    fprintf(stderr, "number of gathers is %lu\n", (unsigned long)nrec);
  ...
  if (verbose)
    (void)fprintf(stderr, "number of header bytes is %d\n", nhdrbytes);
  ...
  if (!sep77) {
    ...
    for (j = 0; j < n2; ++j) {
      ...
      if (inhdrs[j] == ((int *)NULL)) seperr("Unable to allocate memory!\n");
    }
  } else {
    nbytesHdr = 0;
  }
  ...
  for (irec = 0; irec < nrec;) {
    irec++;
    if (verbose && irec == nrec)
      (void)fprintf(stderr, "read last gather, no. %lu\n", irec);
    ...
    for (k = 0; k < n2; ++k) {
      ...
      if (!sep77) {
        ...
        if (0 != istat)
          seperr("header values retrieval for record %d  failed!\n", itemp);
      }
      ...
      if (!sep77) {
        ...
        for (k = 0; k < n2; ++k) {
          for (j = 0; j < nkeys;) {
            ...
            if (keytype[j] == FLOATKEY)
              fprintf(outfp, "%-g", ((float *)(inhdrs[k]))[j]);
            j++;
            putc(',', outfp);
          }
          for (j = 0; j < n1;) {
    ...
    if (wrapup) {
      nrec = irec;
      (void)fprintf(stderr,
                    "User interrupt - processing only first %lu records.\n",
                    nrec);
    }

    idims[2]++;
    for (j = 2; j < (ndims - 1); ++j) {
      if (idims[j] == dimlist[j]) {
        idims[j] = 0;
        idims[j + 1]++;
      }
    }
  ...
  if (dosuper && ndims > 2) {
    ...
    if (sgfp == ((FILE *)NULL)) {
      ...
      (void)fprintf(stderr,
                    "problem opening supergather file \"%s\" ... exiting\n",
                    supergather);
      goto nosupergather;
    }
