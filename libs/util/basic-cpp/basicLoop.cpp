...
{
  if (jw < 1)
    throw SEPException("J can not be less than 1");
  if (fw < 0)
    throw SEPException("f can not be less than 0");
  if (fw + jw * (nw - 1) > n - 1)
    throw SEPException(
...
void blockIO::loopDataInOut(std::shared_ptr<SEP::genericReg> in,
...
void blockIOReg::loopData(std::shared_ptr<SEP::genericReg> in,
                          std::shared_ptr<SEP::genericReg> out)
{
...
void SEP::loop::blockIORegPipe::setupPipe(
    std::shared_ptr<genericReg> inF, std::shared_ptr<genericReg> outF,
