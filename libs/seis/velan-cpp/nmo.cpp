...
std::vector<float> nmoRegCube::getOffSq(const std::shared_ptr<regSpace> in) {
  ...
  for (int ioff = 0; ioff < _ioffAxes.size(); ioff++) {
...
void nmo::applyIt(std::shared_ptr<regSpace> in, std::shared_ptr<regSpace> out) {
  ...
  float oldx = cmpx[0], oldy;

  std::vector<float> ttn(_nt);
  std::cerr << "AAA 4" << std::endl;
  ...
  for (long long itr = 0; itr < n123 / (long long)_nt; itr++) {
    std::cerr << " loop " << itr << std::endl;
    ...
    std::cerr << " loow2 " << itr << std::endl;

    for (int it = 0; it < _nt; it++) inp[it] = inv[_nt * itr + it + 4];
    std::cerr << " loo4w2 " << itr << std::endl;

    float *outp = &outv[itr * _nt];
    *outp = 0;
    std::cerr << " lo5ow2 " << itr << std::endl;

    std::cerr << " w hR " << ttn.size() << "=ttnsize " << itr
    ...
    int itmute;
    std::cerr << " yyy " << itr << std::endl;

    for (itmute = 0;
         itmute < _nt - 1 && ttn[itmute + 1] - ttn[itmute] < _osmute; ++itmute)
      ;
