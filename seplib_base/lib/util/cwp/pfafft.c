...
npfa
 DESCRIPTION
 valid n for complex-to-complex PFA is returned
 
 USAGE
int npfa (int nmin);

Input PARAMETER
 nmin	 - int 	lower bound on returned value (see notes below)

Return Value
 int = valid n for prime factor fft
...
Description
Optimal n for complex-to-complex PFA is returned
...
Input Prameter
nmin - int		lower bound on returned value (see notes below)
nmax - int		desired (but not guaranteed) upper bound on returned value
...
Description
Valid n for real-to-complex/complex-to-real PFA is returned
...
Input Parameter
nmin - int		lower bound on returned value

Return Value
int =	valid n for real-to-complex/complex-to-real prime factor fft
...
Description
Optimal n for real-to-complex/complex-to-real PFA is returned
...
DESCRIPTION
1D PFA complex to complex

Input Parameter
isign		 -int       sign of isign is the sign of exponent in fourier kernel
n		     -int       length of transform (see notes below)
z		     -complex[] array[n] of complex numbers to be transformed in place
...
isign   -int       sign of isign is the sign of exponent in fourier kernel
n       -int       length of transform (see notes below)
cz      -complex[] array[n/2+1] of complex values (may be equivalenced to rz)

Output Parameters
rz      float[]    array[n] of real values (may be equivalenced to cz)
...
Input Parameter
isign - int      sign of isign is the sign of exponent in fourier kernel
n     - int      length of transform; must be even (see notes below)
rz    - float[]  array[n] of real values (may be equivalenced to cz)
...
Input Parameters
isign   - int     	sign of isign is the sign of exponent in fourier kernel
n       - int     	number of complex elements per transform (see notes below)
nt      - int      	number of transforms
k       - int    	  stride in complex elements within transforms
kt      - int    	  stride in complex elements between transforms
z       - complex[] array of complex elements to be transformed in place

Output Parameters
z		    - complex[] array of commplex elements transformed
...
Input Parameter
isign   -int      	sign of isign is the sign of exponent in fourier kernel
idim    -int      	dimension to transform, either 1 or 2 (see notes)
n1      -int      	1st (fast) dimension of array to be transformed (see notes)
n2      -int      	2nd (slow) dimension of array to be transformed (see notes)
z       -complex[] 	array[n2][n1] of complex elements to be transformed in place
...
Input Parameters
isign   -int       sign of isign is the sign of exponent in fourier kernel
idim    -int       dimension to transform, which must be either 1 or 2 
                   (see notes)
n1      -int       1st (fast) dimension of array to be transformed (see notes)
n2      -int       2nd (slow) dimension of array to be transformed (see notes)
cz      -complex[]  array of complex values (may be equivalenced to rz)

Output Parameter
rz      -float[]    array of real values (may be equivalenced to cz)
...
Input Parameters
isign  -int      sign of isign is the sign of exponent in fourier kernel
idim   -int      dimension to transform, which must be either 1 or 2 (see notes)
n1     -int      1st (fast) dimension of array to be transformed (see notes)
n2     -int      2nd (slow) dimension of array to be transformed (see notes)
rz     -float[]  array of real values (may be equivalenced to cz)
...
*****************************************************************************/
/**************** end self doc ********************************/
/*Sean Crawley, changed to seplib.h*/
#include <sulib.h>
...
*****************************************************************************/
{
	...
	int nleft,jfax,ifac,jfac,jinc,jmax,ndiv,m,mm,mu,l;
...
#if NeedFunctionPrototypes
_XFUNCPROTOBEGIN
void pfarc (int isign, int n, float *rz, float *kz)
_XFUNCPROTOEND
#else
void pfarc (isign, n, rz, kz)
int isign;
int n;
float *rz,*kz;
#endif
...
Output:
z		array of commplex elements transformed
******************************************************************************
Notes:
n must be factorable into mutually prime factors taken 
from the set {2,3,4,5,7,8,9,11,13,16}.  in other words,
...
*****************************************************************************/
{
    ...
    int nleft,jfax,ifac,jfac,iinc,imax,ndiv,m,mm,mu,l,istep,jstep,
...
#endif /* TEST */
/*  $Id: pfafft.c,v 1.1.1.1 2004/03/25 06:37:24 cvs Exp $ */
