...
int nargs,ith,nsize;
  ...
  MPI_Comm_rank(MPI_COMM_WORLD, &ith);

  narg=sepxargc; 
  MPI_Bcast(&narg,1,MPI_INT, 0, MPI_COMM_WORLD);
  for(iarg=0; iarg< narg; iarg++){
    if(ith==0) nsize=1+(int)strlen(sepxargv[iarg]);
    MPI_Bcast(&nsize,1,MPI_INT, 0, MPI_COMM_WORLD);
MPI_Barrier(MPI_COMM_WORLD);
    if(ith==0) strcpy(temp_ch,sepxargv[iarg]);
    MPI_Bcast(temp_ch,nsize,MPI_CHAR, 0, MPI_COMM_WORLD);
MPI_Barrier(MPI_COMM_WORLD);
    if(ith!=0) getch_add_string(temp_ch);
  }
  if(ith!=0){
