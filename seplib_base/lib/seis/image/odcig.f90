module odcig_mod
  ...
  integer(kind=dbl), pointer, dimension(:),private ::  planF,  planI
...
!
    call  sfftw_plan_dft(planI(i),3,ni,cf(1,1,1,i),cf(1,1,1,i)   ,fFTW_BACKWARD  &
    ,    fFTW_ESTIMATE + fFTW_THREADSAFE )

!    call fftwnd_f77_create_plan(planF,1,(/ni(1)/),fFTW_FORWARD &
!    ,    fFTW_ESTIMATE + fFTW_THREADSAFE + fFTW_IN_PLACE)

    call  sfftw_plan_dft(planF(i),1,ni,cf(1,1,1,i),cf(1,1,1,i)   ,fFTW_BACKWARD  &
    ,    fFTW_ESTIMATE + fFTW_THREADSAFE )
