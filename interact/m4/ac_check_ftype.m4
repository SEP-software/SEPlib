...
if ( eval "grep -i vast in.v7") ; then
	...
	if ( eval "grep -i numerical in.v1") ; then
		...
		if ( eval "grep -i g77 in.v2") ; then
			$2="$[$2] -Dgnu -Df2cFortran"
			$5="$[$5] -ffixed-line-length-none -ff90 -fdollar-ok -fargument-noalias -fno-globals "
			$4="$[$4] -lm -lieee "
		fi
		if ( eval "grep -i egcs in.v2") ; then
			$2="$[$2] -Dgnu -Df2cFortran"
			$3="$[$3] -ffixed-line-length-none -ff90 -fdollar-ok -fargument-noalias -fno-globals "
			$4="$[$4] -lf2c -lm -lieee -lf2c"
		fi
		if ( eval "grep -i gcc in.v2") ; then
			$2="$[$2] -Dgnu"
			$5="$[$5] -ffixed-line-length-none -ff90 -fdollar-ok -fargument-noalias -fno-globals "
			$4="$[$4] -lf2c -lm -lieee -lf2c"
		fi
...
rm -f in.v1 in.v2 in.v3 in.v4 in.v5 in.v6 in.v7 in.v8
])
