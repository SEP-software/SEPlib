...
RenderInit (void)
	...
	GETPARINT ("interpolate","d",&render->interp);
	GETPARINT ("fence_transp","d",&render->fence_transp);
	GETPARINT ("vol_transp","d",&render->vol_transp);
	render->image = 0;
	render->shadow = 0;
	render->zbuffer = 0;
	return (render);
...
void RenderHorz (Data data,Map hmap,Map vmap,Map zmap,Map map4,Map map5,int h0,int v0,Render render,int margins[],RenderAttr attr)
	...
	if (attr->mark) PlaneSet (hmap,vmap,zmap,map4,map5,render->h0,render->v0,render->nh,render->nv,render,margins,attr);
	else {
	...
	if (attr->image) {
		...
		else if (render->interp) RENDER_INTERP_HORZ;
		else RENDER_BASIC_HORZ;
	...
	if (attr->lines) {
		...
		DrawLine (x+attr->skew*MapSize(vmap),v0,x,v0+MapSize(vmap),ERASE);
		/* draw horizontal */
		x = h0 + attr->skew * (MapSize(vmap) -
...
void RenderVert (Data data,Map hmap,Map vmap,Map zmap,Map map4,Map map5,int h0,int v0,Render render,int margins[],RenderAttr attr)
	...
	if (attr->lines) {
		...
		y = v0 + MapInverse(vmap,MapPrev(vmap));
		DrawLine (h0,y,h0+MapSize(hmap),y-attr->skew*MapSize(hmap),ERASE);
		/* erase vertical line */
	...
	if (attr->axes&AXIS_BASE) {
		...
		if (attr->skew) DrawText (x,y+TIC_LENGTH,TEXT_V0|TEXT_H0,
...
void RenderDeep (Data data,Map hmap,Map vmap,Map zmap,Map map4,Map map5,int h0,int v0,Render render,int margins[],RenderAttr attr)
	...
	RenderHorz (data,&hmap1,&vmap1,&zmap1,map4,map5,h0,v0,render,margins,attr);
...
void RenderMap (Render render)
	...
	for (id=0; id<dbase; id++) render->cmap[id] = cback;
	for (id=dbase+dsize; id<128; id++) render->cmap[id] = cback;
	for (id=128; id<256; id++) render->cmap[id] = cmark;
	/* marked data, i.e. > dsize, revert to maximum value */
	for (value=((cbase+csize)/2)*2+2; id<256; id++) render->cmap[id] = value;
