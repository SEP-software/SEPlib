...
#define RENDER_ATTR(_transp,_skew,_mark,_zbuffer,_image,_axes,_shadow,_lines,_box,_pick) \
	...
	_attr_->mark = (_mark);\
	/*printf ("image=%d shadow=%d zbuffer=%d box=%d axes=%d lines=%d pick=%d skew=%d transp=%d mark=%d\n",_image,_shadow,_zbuffer,_box,_axes,_lines,_pick,_skew,_transp,_mark)*/
Message message;
...
Data data;
	...
	if (GETPARSTRING ("style","s",option)) {
	...
	else GETPARINT ("style","d",&view->style);
	GETPARINT ("wide","d",&view->wide);
	GETPARINT ("hite","d",&view->hite);
	switch (view->style) {
	case VIEW_TRANSP:
	case VIEW_SIDE:
	...
	if (GETPARSTRING ("movie","s",option)) {
		if (!strcmp (option,"in")) {
			view->movie = MOVIE_FRONT;
	...
	if (GETPARSTRING ("shape","s",option)) {
	...
	view->showpicks = DRAW;
	GETPARINT ("showpicks","d",&view->showpicks);
	view->nacross = 1;
	GETPARINT ("across","d",&view->nacross);
	view->ndown = 1;
	GETPARINT ("down","d",&view->ndown);
	view->dacross = 1;
	GETPARINT ("delta","d",&view->dacross);
	view->rate = VIEW_RATE_TENTHS;
	GETPARINT ("rate","d",&view->rate);
	view->fence = DRAW_ALL;
	GETPARINT ("fence","d",&view->fence);
	...
	if (GETPARSTRING ("orient","s",option)) {
...
/*				ViewSetShape (SHAPE_FIT);*/
		...
		break;
	case VIEW_FRONT:
		ViewSingleSize (view->map[AXIS_ACROSS],view->map[AXIS_DOWN],view->map[AXIS_DEEP]);
		break;
	case VIEW_SIDE:
		ViewSingleSize (view->map[AXIS_DEEP],view->map[AXIS_DOWN],view->map[AXIS_ACROSS]);
		break;
	case VIEW_TOP:
		ViewSingleSize (view->map[AXIS_ACROSS],view->map[AXIS_DEEP],view->map[AXIS_DOWN]);
		break;
	case VIEW_XSEC:
...
int mode;
	...
	if (view->showpicks && view->style != VIEW_TRANSP) PikDraw (NO_INDEX,ERASE);
	switch (view->style) {
	case VIEW_CUBE:
	...
	case VIEW_FRONT:
	case VIEW_XSEC:
	RENDER_ATTR(0,0,mode==DRAW_ALL,0,(mode&DRAW_IMAGE),(AXIS_LEFT|AXIS_BASE)*!MovieRun(),!MovieRun(),0,!MovieRun(),view->showpicks);
	RenderHorz (data,view->map[AXIS_ACROSS],view->map[AXIS_DOWN],view->map[AXIS_DEEP],
	...
	case VIEW_THREE:
		...
		strcat (message,"                                   ");
		DrawText (view->margins[MARGIN_LEFT],0,TEXT_H0|TEXT_V0,DataTitle(data));
	...
	if (view->showpicks && view->style != VIEW_TRANSP) PikDraw (NO_INDEX,DRAW);
...
View view;
	...
	for (panel=0; panel<view->nacross*view->ndown; panel++) {
		...
		RenderHorz (data,view->map[AXIS_ACROSS],
			...
			DrawText (ih,iv+2,TEXT_H0|TEXT_V0,AxisScript(ViewDataAxis(view,AXIS_DEEP),frame));
			}
		else	{
...
int mode;
	...
	if (!view) return;
	/* front */
	RENDER_ATTR(0,0,mode==DRAW_ALL,0,(mode&DRAW_FRONT)*(mode&DRAW_IMAGE),(AXIS_LEFT|AXIS_BASE)*!MovieRun(),!MovieRun(),1,!MovieRun(),view->showpicks);
	RenderHorz (data,view->map[AXIS_ACROSS],view->map[AXIS_DOWN],
		...
		render,view->margins,_attr_);
	/* side */
	RENDER_ATTR(0,1,mode==DRAW_ALL,0,(mode&DRAW_SIDE)*(mode&DRAW_IMAGE),(AXIS_BASE)*!MovieRun(),!MovieRun(),1,!MovieRun(),view->showpicks);
	RenderVert (data,view->map[AXIS_DEEP],view->map[AXIS_DOWN],
		...
		render,view->margins,_attr_);
	/* top */
	RENDER_ATTR(0,1,mode==DRAW_ALL,0,(mode&DRAW_TOP)*(mode&DRAW_IMAGE),(AXIS_LEFT)*!MovieRun(),!MovieRun(),1,!MovieRun(),view->showpicks);
	RenderHorz (data,view->map[AXIS_ACROSS],view->map[AXIS_DEEP],
		view->map[AXIS_DOWN],view->map[AXIS_4D],view->map[AXIS_5D],
		view->margins[MARGIN_LEFT],
...
int mode;
	...
	/* front */
	if (mode & DRAW_FRONT) {
	RENDER_ATTR(0,0,mode==DRAW_ALL,1,mode&DRAW_FRONT,0,!MovieRun(),0,0,0);
	RenderHorz (data,view->map[AXIS_ACROSS],view->map[AXIS_DOWN],
		view->map[AXIS_DEEP],view->map[AXIS_4D],view->map[AXIS_5D],
	...
	/* side */
	if (mode & DRAW_SIDE) {
	RENDER_ATTR(0,1,mode==DRAW_ALL,1,mode&DRAW_SIDE,0,!MovieRun(),0,0,0);
	RenderVert (data,view->map[AXIS_DEEP],view->map[AXIS_DOWN],
		view->map[AXIS_ACROSS],view->map[AXIS_4D],view->map[AXIS_5D],
	...
	/* top */
	if (mode & DRAW_TOP) {
	RENDER_ATTR(0,1,mode==DRAW_ALL,1,mode&DRAW_TOP,0,!MovieRun(),0,0,0);
	RenderHorz (data,view->map[AXIS_ACROSS],view->map[AXIS_DEEP],
		view->map[AXIS_DOWN],view->map[AXIS_4D],view->map[AXIS_5D],
		view->margins[MARGIN_LEFT],
...
int mode;
	...
	RenderHorz (data,view->map[AXIS_ACROSS],view->map[AXIS_DOWN],
		...
		render,view->margins,_attr_);
	/* side */
	RENDER_ATTR(0,0,mode==DRAW_ALL,0,(mode&DRAW_SIDE)*(mode&DRAW_IMAGE),(AXIS_BASE)*!MovieRun(),!MovieRun(),1,!MovieRun(),view->showpicks);
	RenderVert (data,view->map[AXIS_DEEP],view->map[AXIS_DOWN],
		view->map[AXIS_ACROSS],view->map[AXIS_4D],view->map[AXIS_5D],
		view->margins[MARGIN_LEFT]+MapSize(view->map[AXIS_ACROSS])+GAP+1,
		MapSize(view->map[AXIS_DEEP])+view->margins[MARGIN_TOP]+GAP,
		render,view->margins,_attr_);
	/* top */
	RENDER_ATTR(0,0,mode==DRAW_ALL,0,(mode&DRAW_TOP)*(mode&DRAW_IMAGE),(AXIS_LEFT)*!MovieRun(),!MovieRun(),1,!MovieRun(),view->showpicks);
	RenderHorz (data,view->map[AXIS_ACROSS],view->map[AXIS_DEEP],
		view->map[AXIS_DOWN],view->map[AXIS_4D],view->map[AXIS_5D],
		view->margins[MARGIN_LEFT],
		view->margins[MARGIN_TOP],
...
View view;
	...
	UITimer(1,ViewDrawTranspCallback);
...
ViewDrawTranspCallback ()
	...
	case MOVIE_SIDE:
		...
		RenderVert (data,view->map[AXIS_DEEP],view->map[AXIS_DOWN],
			view->map[AXIS_ACROSS],view->map[AXIS_4D],view->map[AXIS_5D],
			view->margins[MARGIN_LEFT]+MapFrame1(view->map[AXIS_ACROSS]),
			MapSize(view->map[AXIS_DEEP])+view->margins[MARGIN_TOP],
			render,view->margins,_attr_);
		if (MapFrame1(view->map[AXIS_ACROSS]) && MapFrame1(view->map[AXIS_ACROSS])%block == 0) RenderDraw ();
...
ViewDrawMovie ()
	...
	map = ViewMovieMap (view);
	MapNextFrame (map);
	if (MovieCache()) mode = DrawPixmap(MapFrame(map));
	mode = (1 - mode) * DRAW_IMAGE;
	switch (view->movie) {
	case MOVIE_TOP: ViewDraw (view,DRAW_TOP|mode); break;
	case MOVIE_SIDE: ViewDraw (view,DRAW_SIDE|mode); break;
	case MOVIE_FRONT: ViewDraw (view,DRAW_FRONT|mode); break;
		}
	if (mode) {
		RenderDraw ();
		if (MovieCache()) {
	...
	if (MovieRun()) UITimer(MovieDelay()*10+1,ViewDrawMovie);
#endif
	UIWakeup();
	}
...
/*	PlaneFind (AxisDir(MapAxis(map)),NO_INDEX);*/
	DrawFreePixmaps();
	if (view->style == VIEW_TRANSP) ViewDraw (view,DRAW_ALL);
	}
...
int index,mode;
	...
	switch (view->movie) {
	case MOVIE_TOP: ViewDraw (view,DRAW_TOP); break;
	case MOVIE_SIDE: ViewDraw (view,DRAW_SIDE); break;
	case MOVIE_FRONT: ViewDraw (view,DRAW_FRONT); break;
		}
	RenderDraw ();
	}
...
ViewFramesMiddle ()
	...
	if (!view) return;
	for (imap=1; imap<VIEW_NAXIS; imap++) {
		MapSetFrameBounds (view->map[imap],MapFirst(view->map[imap]),MapLast(view->map[imap]));
		MapSetFrame (view->map[imap],(MapFirst(view->map[imap])+MapLast(view->map[imap]))/2);
		}
	ViewDrawAll ();
	}
...
int a,b;
	...
	if (!view) return;
	if (MapSize(view->map[b]) <= 1 || MapSize(view->map[b]) <= 1) return;
	...
	if (view->style != VIEW_ARRAY) ViewDrawAll ();
	}
...
int x1,y1,x2,y2,hzoom,vzoom;
	...
	if (hzoom && (x1-x2)*(x1-x2) < 25) return;
	if (vzoom && (y1-y2)*(y1-y2) < 25) return;
	/* fetch indices */
	...
	/*
	if (PickSameDir (&pick11,&pick22)) {
		printf ("Pick direction is %s\n",AxisLabel(data->axis[pick11.iaxis[AXIS_DEEP]]));
		}
	if (PickSameFrame (&pick11,&pick22)) {
		iaxis = pick11.iaxis[AXIS_DEEP];
		printf ("Pick frame is %s\n",AxisScript(data->axis[iaxis],pick11.index[iaxis]));
		}
	if ((haxis=PickSharedDir (&pick11,&pick12)) > 0) {
		printf ("Pick horizontal is %s\n",AxisLabel(data->axis[haxis]));
		}
	if ((vaxis=PickSharedDir (&pick11,&pick21)) > 0) {
		printf ("Pick vertical is %s\n",AxisLabel(data->axis[vaxis]));
		}
	printf ("\n");
	*/
	/* window across an array of frames */
	if (view->style == VIEW_ARRAY && PickSameDir(&pick11,&pick22) && !PickSameFrame(&pick11,&pick22)) {
		MapSetFrameBounds (view->map[AXIS_DEEP],pick11.index[haxis],pick22.index[vaxis]);
		UIArrayReset (0);
		UIArrayDraw ();
		return;
		}
	/* within a frame */
	if (!PickSameFrame(&pick11,&pick22)) return;
	...
	*/
	...
	if (hzoom) {
		for (iaxis=1; iaxis<VIEW_NAXIS; iaxis++) {
			if (AxisDir(MapAxis(view->map[iaxis])) == haxis) break;
			}
		map = view->map[iaxis];
	...
	if (vzoom) {
		for (iaxis=1; iaxis<VIEW_NAXIS; iaxis++) {
			if (AxisDir(MapAxis(view->map[iaxis])) == vaxis) break;
			}
		map = view->map[iaxis];
		MapSet (map,MapAxis(map),MapSize(map),
	...
	if (view->shape == SHAPE_TRUE || view->shape == SHAPE_PIXEL) ViewSize (view);
...
View view;
	...
	if (!view) return;
	sprintf (message,"Style: wide=%d hite=%d movie=%d style=%d shape=%d pick=%d across=%d down=%d delta=%d fence=%d rate=%d",
		view->wide,
		view->hite,
		view->movie,
		view->style,
...
ViewOrient0 ()
	...
	for (imap=1; imap<VIEW_NAXIS; imap++) {
		if (AxisDir(MapAxis(map[imap])) == AXIS_DEEP) {
...
int pixels;
	...
	tyme = Tyme() - tyme;
	sprintf (message,"render=%d msec pixels=%d rate=%d pixels/sec",(int)tyme,pixels,(int)(1000.*pixels/tyme));
	UIMessage (message);
	tyme = 0.0;
	}
#endif
