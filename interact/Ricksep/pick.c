...
int PickDecode (int x,int y,PickPoint pick,int print)
	...
	/* no pick made */
	if (!render || !myv || !myv->data) return(0);
	/* no invalid pick */
	...
	if ((int)shadow == NO_INDEX) return(0);
...
*/
	buffer = DataBuffer (myv->data);
/* commented for now probably want this back
	pick->index[DATA_VALUE] = buffer[shadow] - DATA_VALUE_BASE;
*/
	if( pick->index[DATA_VALUE]==NO_INDEX)  pick->index[DATA_VALUE]=2;
	/* print pick in message */
	if (print)  {
		sprintf (message,"dir=%s frame=%d,%d,%d %s=%g %s=%g %s=%g %s=%g %s=%g %s=%g",
		AxisLabel(DataAxis(myv->data,pick->iaxis[AXIS_DEEP])),
		pick->index[pick->iaxis[AXIS_DEEP]],
		pick->index[pick->iaxis[AXIS_4D]],
		pick->index[pick->iaxis[AXIS_5D]],
		AxisLabel(DataAxis(myv->data,DATA_AXIS1)),
		AxisValue(DataAxis(myv->data,DATA_AXIS1),pick->index[DATA_AXIS1]),
		AxisLabel(DataAxis(myv->data,DATA_AXIS2)),
		AxisValue(DataAxis(myv->data,DATA_AXIS2),pick->index[DATA_AXIS2]),
		AxisLabel(DataAxis(myv->data,DATA_AXIS3)),
		AxisValue(DataAxis(myv->data,DATA_AXIS3),pick->index[DATA_AXIS3]),
		AxisLabel(DataAxis(myv->data,DATA_AXIS4)),
		AxisValue(DataAxis(myv->data,DATA_AXIS4),pick->index[DATA_AXIS4]),
		AxisLabel(DataAxis(myv->data,DATA_AXIS5)),
		AxisValue(DataAxis(myv->data,DATA_AXIS5),pick->index[DATA_AXIS5]),
		AxisLabel(DataAxis(myv->data,DATA_VALUE)),
		AxisValue(DataAxis(myv->data,DATA_VALUE),pick->index[DATA_VALUE]));
		UIMessage (message);
		}
	return(0);
	}
...
void PickDecodeShadow (Shadow_ shadow,int *index)
    ...
    if(AxisSize(DataAxis(myv->data,iaxis))>1){
		  index[iaxis] = shadow / AxisStride_s(DataAxis(myv->data,iaxis));
		  shadow = shadow % AxisStride_s(DataAxis(myv->data,iaxis));
     }
     else index[iaxis]=0;
		}
	}
...
/* find a pick list */
PickLine PickFind (int dir3,int frame3,int dir4,int frame4,int dir5,int frame5)
	{
	int iset;
...
void PickDraw (PickLine pickline,int draw)
	...
	if ((plane = PlaneFind(PICKDIR(pickline->,AXIS_DEEP),PICKFRAME0(pickline->,AXIS_DEEP),
		...
		PICKDIR(pickline->,AXIS_5D),PICKFRAME0(pickline->,AXIS_5D),0)) == 0) return;
...
void PickRead ()
	...
	if (!myv->data) return;
	if (rick_getch ("annotate","s",pickfile) == 0) return;
...
/*
	...
	if (n1 != AxisSize(DataAxis(myv->data,DATA_AXIS1))
	 || n2 != AxisSize(DataAxis(myv->data,DATA_AXIS2)) 
	 || n3 != AxisSize(DataAxis(myv->data,DATA_AXIS3)) 
	 || n4 != AxisSize(DataAxis(myv->data,DATA_AXIS4)) 
	 || n5 != AxisSize(DataAxis(myv->data,DATA_AXIS5))  ){
			...
			if (picklist[iset]->npick < NPICK) {
				...
				for (i=1; i<DATA_NAXIS; i++) {
					picklist[iset]->index[picklist[iset]->npick][i] =
						AxisIndex(DataAxis(myv->data,i),value[i]);
					}
...
/*
	fprintf (fd,"Dimensions: %d %d %d %d %d\n",
		...
		AxisSize(DataAxis(myv->data,DATA_AXIS4)),
		AxisSize(DataAxis(myv->data,DATA_AXIS5)));
*/
	...
	AxisLabel(DataAxis(myv->data,DATA_AXIS4)),
	AxisLabel(DataAxis(myv->data,DATA_AXIS5)),
	AxisLabel(DataAxis(myv->data,DATA_AXIS0)));
	for (iset=0, isave=0; iset < PICKNLIST; iset++) {
		if (picklist[iset] && picklist[iset]->npick > 0) {
			fprintf (fd,"Pick# %d axis# %d %d %d %d %d %s\n",
				...
				PICKDIR(picklist[iset]->,AXIS_4D),
				PICKDIR(picklist[iset]->,AXIS_5D),
				AxisScript(DataAxis(myv->data,PICKDIR(picklist[iset]->,AXIS_DEEP)),
				(int)(picklist[iset]->iaxis/*index*/[AXIS_DEEP])));
			for (ipick=0; ipick<picklist[iset]->npick; ipick++) {
				fprintf (fd,"%10g %10g %10g %10g %10g %10g\n",
				AxisValue (DataAxis(myv->data,DATA_AXIS1),
					picklist[iset]->index[ipick][DATA_AXIS1]),
				AxisValue (DataAxis(myv->data,DATA_AXIS2),
					picklist[iset]->index[ipick][DATA_AXIS2]),
				AxisValue (DataAxis(myv->data,DATA_AXIS3),
					picklist[iset]->index[ipick][DATA_AXIS3]),
				AxisValue (DataAxis(myv->data,DATA_AXIS4),
					picklist[iset]->index[ipick][DATA_AXIS4]),
				AxisValue (DataAxis(myv->data,DATA_AXIS5),
...
/* set pick frame */
int PickSetFrame (int index)
	{
	PickLine pickline;
	int iset;

	if (index < 0 || index > PickSize()) return(0);
	for (iset=0; iset<PICKNLIST; iset++) {
	...
	if (iset == PICKNLIST) return(0);
	ViewSetMovie (PICKDIR(pickline->,AXIS_DEEP));
	ViewSetFrame ((int)(pickline->iaxis/*index*/[AXIS_DEEP]),FRAME_ACTUAL);
	PickDraw (pickline,DRAW);
  return(0);
	}
...
void PickSavePar ()
	...
	sprintf (message,"Pick: nset=%d pickdir=%d pickframe=%d npick=%d pick=%s",
		...
		(int)(lastpick->iaxis/*index*/[AXIS_DEEP]),
		lastpick->npick,
		pickfile);
