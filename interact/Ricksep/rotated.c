    ...
    int bg;
{
    ...
    my_gc=XCreateGC(dpy, drawable,(unsigned long) NULL, 0);
	...
	depth_one_gc=XCreateGC(dpy, empty_stipple, (unsigned long)NULL, 0);
	...
	if(XGetGCValues(dpy, gc, 
		...
		/* create a GC */
		depth_one_gc=XCreateGC(dpy, new_bitmap,(unsigned long) NULL, 0);
    ...
    int align;
{
    ...
    font_gc=XCreateGC(dpy, canvas, (unsigned long) NULL, 0);
