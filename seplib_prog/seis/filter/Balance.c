...
=item oper   -  integer 
      ...
      [0]:  compute balance factors from input data, and apply them to create output.
      [1]:  compute balance factors from input data, and output the balance factors.
      [2]:  read in balance factors from auxiliary dataset (bal=),
              apply them to main input, and output the trace-balanced data.

=item rms      -  real 

      [2000]:  desired output rms level. If rms=0, balance factors will be 
               computed to match the rms level of the first live trace.
...
=head1 EXAMPLE
    ...
    Balance < in.H oper=0 rms=2000> out.H
        Compute balance factors in a 2000 ms window and apply to the data.
...
#define MY_SEP_DOC \
 ...
 sep_add_doc_line("    Balance - Perform trace balancing");\
 sep_add_doc_line("");\
 sep_add_doc_line("SYNOPSIS");\
 sep_add_doc_line("        Balance < in.H bal= oper=0 rms=2000 > out.H");\
 sep_add_doc_line("");\
 sep_add_doc_line("INPUT PARAMETERS");\
 sep_add_doc_line("    oper - integer");\
 sep_add_doc_line("              [0]:  compute balance factors from input data, and apply them to create output.");\
 sep_add_doc_line("              [1]:  compute balance factors from input data, and output the balance factors.");\
 sep_add_doc_line("              [2]:  read in balance factors from auxiliary dataset (bal=),");\
 sep_add_doc_line("                      apply them to main input, and output the trace-balanced data.");\
 sep_add_doc_line("");\
 sep_add_doc_line("    rms - real");\
 sep_add_doc_line("              [2000]:  desired output rms level. If rms=0, balance factors will be ");\
 sep_add_doc_line("                       computed to match the rms level of the first live trace.");\
 sep_add_doc_line("");\
 sep_add_doc_line("    n1,n2,n3- int");\
 sep_add_doc_line("              standard seplib pars");\
 sep_add_doc_line("");\
 sep_add_doc_line("    bal - char");\
 sep_add_doc_line("              name of auxiliary file containing balance factors (if oper==2)");\
 sep_add_doc_line("");\
 sep_add_doc_line("DESCRIPTION");\
 sep_add_doc_line("    Trace balancing of an entire trace (no windowing)");\
 sep_add_doc_line("");\
 sep_add_doc_line("EXAMPLE");\
 sep_add_doc_line("        Balance < in.H bal=mybal.txt oper=2 > out.H");\
 sep_add_doc_line("           Apply balance factors contained in file mybal.txt");\
 sep_add_doc_line("");\
 sep_add_doc_line("        Balance < in.H oper=0 rms=2000> out.H");\
 sep_add_doc_line("            Compute balance factors in a 2000 ms window and apply to the data.");\
 sep_add_doc_line("");\
