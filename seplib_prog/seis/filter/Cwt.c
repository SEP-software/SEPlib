...
sep_add_doc_line("    Cwt - Complex continuous wavelet transform of seismic traces");
sep_add_doc_line("");
sep_add_doc_line("SYNOPSIS");
sep_add_doc_line("          Cwt < tdata.H > tfdata.H  [optional parameters]");
sep_add_doc_line("");
sep_add_doc_line("INPUT PARAMETERS");
sep_add_doc_line("    noct - integer");
sep_add_doc_line("                  [5]:  Number of octaves");
sep_add_doc_line("");
sep_add_doc_line("    nv - integer");
sep_add_doc_line("                  [10]:  Number of voices per octove");
sep_add_doc_line("");
sep_add_doc_line("    fmax - real");
sep_add_doc_line("                  [Nyq]:  Highest frequency in transform");
sep_add_doc_line("");
sep_add_doc_line("    p - real");
sep_add_doc_line("                  [.5]:  Power of scale value normalizing CWT ");
sep_add_doc_line("                   0 - for amp-preserved spec. decomp.");
sep_add_doc_line("");
sep_add_doc_line("    c - real");
sep_add_doc_line("                  [1/(2*fmax)]:   Time-domain inverse gaussian ");
sep_add_doc_line("                  damping parameter  (bigger c means more wavelet");
sep_add_doc_line("                   oscillations, , default gives minimal oscillations)");
sep_add_doc_line("");
sep_add_doc_line("    out_type - integer");
sep_add_doc_line("                  [1]:  Output amplitude of CWT; ");
sep_add_doc_line("                  2- output phase");
sep_add_doc_line("                  3- output real");
sep_add_doc_line("                  4 -output imaginary,");
sep_add_doc_line("                  5- output complex");
sep_add_doc_line("");
sep_add_doc_line("    k - integer");
sep_add_doc_line("                  [1]:  Use complex Morlet as wavelet transform kernel");
sep_add_doc_line("                   2 - use Fourier kernel");
sep_add_doc_line("");
sep_add_doc_line("    fs - integer");
sep_add_doc_line("                  [1]:  Use dyadic freq sampling ");
sep_add_doc_line("                    (CWT standard honors noct, nv)");
sep_add_doc_line("                  2- use linear freq sampling");
sep_add_doc_line("");
sep_add_doc_line("EXAMPLES");
sep_add_doc_line("      This generates amplitude spec of the CWT impulse response (IR).");
sep_add_doc_line("         Spike k1=1 n1=125 | Cwt |Grey|Tube");
sep_add_doc_line("     Real part of Fourier IR with linear freq sampling ");
sep_add_doc_line("         Spike k1=1 n1=125 | Cwt  k=2 fs=2 out_type=3 |Grey|Tube,");
sep_add_doc_line("");
sep_add_doc_line("Notes");
sep_add_doc_line("    1. Total number of scales: nscales = noct*nv 2. Each input trace spawns");
sep_add_doc_line("    nscales output traces (n2) 3. Lowest frequency in the transform is");
sep_add_doc_line("    fmax/( 2^(noct-1/nv) )");
sep_add_doc_line("");
...
sep_add_doc_line("");
	...
	doc(SOURCE);
       ...
       nt=n[0];
		...
		if (k != 1 && k != 2) {
			k = 1;
			sepwarn("Bad kernal wavelet choice ... use Morlet");
		}
		...
		if (verbose==1) sepwarn("fmax = %g, fmin = %g\n", fmax, fmax/a[na-1]);
		if (verbose==1) sepwarn("scale_index / scale_value / frequency");
		if (verbose==1) {
			for (ia=0; ia<na; ia++) {
