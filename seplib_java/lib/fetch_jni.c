/* native method for Fetch class static methods fetch() */
#include "../include/sitedef.h"
#include "edu_stanford_sep_seplib_corelibs_sep_Fetch.h"
#include <seplib.h>
#if HAVE_STRING_H
#include <string.h>
#endif
...
#define BODYCODE(Typeval, typeval, mytypeval) \
  ...
  rc = fetch((char *)name, (char *)format, myVals); \
  if(rc < 1) return(rc); \
  ...
  if(rc > sizeof(myVals)/sizeof(myVals[0])) { /* throw an overflow exception */\
     ...
     if(exceptCls != ((jclass) NULL)) { \
         (*env)->ThrowNew(env,exceptCls,"fetch array overrun\n"); \
     } \
     return(rc); \
  } \
  /* Copy fetch result to output argument */ \
