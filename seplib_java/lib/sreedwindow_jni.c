/* native method for SreedWindow class static methods sreed_window() */
#include "../include/sitedef.h"
#include "edu_stanford_sep_seplib_corelibs_sep_SreedWindow.h"
#include <seplib.h>
#if HAVE_STRING_H
#include <string.h>
#endif
...
#define BODYCODE(Typeval,typeval) \
  ...
  /* sanity checks */ \
   ...
   for(i=0, nbytesToRead=esize; i<n_dim; ++i) nbytesToRead *= n_wind[i]; \
   nInBuf = (*env)->GetArrayLength(env, jvals); \
   if((nInBuf*sizeof(j ## typeval)) < nbytesToRead) return (rc); \
   ...
   rc = (jint) sreed_window(tag,&n_dim,n_cube,n_wind,f_wind,j_wind,esize,vals); \
