/* native method for Hetch class static methods hetch() */
#include "../include/sitedef.h"
#include "edu_stanford_sep_seplib_corelibs_sep_Hetch.h"
#include <seplib.h>
#if HAVE_STRING_H
#include <string.h>
#endif
...
#define BODYCODE(Typeval, typeval, mytypeval) \
  ...
  rc = hetch((char *)name, (char *)format, myVals); \
  if(rc < 1) return(rc); \
  ...
  if(rc > sizeof(myVals)/sizeof(myVals[0])) { /* throw an overflow exception */\
     ...
     if(exceptCls != ((jclass) NULL)) { \
         (*env)->ThrowNew(env,exceptCls,"hetch array overrun\n"); \
     } \
     return(rc); \
  } \
  /* Copy hetch result to output argument */ \
...
/*ARGSUSED*/
JNIEXPORT jint JNICALL Java_edu_stanford_sep_seplib_corelibs_sep_Hetch_hetch__Ljava_lang_String_2Ljava_lang_String_2_3I
...
/*ARGSUSED*/
JNIEXPORT jint JNICALL Java_edu_stanford_sep_seplib_corelibs_sep_Hetch_hetch__Ljava_lang_String_2Ljava_lang_String_2_3F
...
/*ARGSUSED*/
JNIEXPORT jint JNICALL Java_edu_stanford_sep_seplib_corelibs_sep_Hetch_hetch__Ljava_lang_String_2Ljava_lang_String_2_3D
...
/*ARGSUSED*/
JNIEXPORT jint JNICALL Java_edu_stanford_sep_seplib_corelibs_sep_Hetch_hetch__Ljava_lang_String_2Ljava_lang_String_2_3B
